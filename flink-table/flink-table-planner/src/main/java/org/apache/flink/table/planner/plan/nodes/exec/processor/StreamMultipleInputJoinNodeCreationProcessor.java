 ...
 * @author Quentin Qiu
 */
public class StreamMultipleInputJoinNodeCreationProcessor implements ExecNodeGraphProcessor {

    private final boolean isStreaming;

    public StreamMultipleInputJoinNodeCreationProcessor(boolean isStreaming) {
    ...
    public ExecNodeGraph process(ExecNodeGraph execGraph, ProcessorContext context) {
        ...
        // remove exchange node between joins
        removeExchangeBetweenJoins(orderedWrappers);
        ...
        optimizeMultipleInputJoinGroups(orderedWrappers, context);
    ...
    /*only for same key joins*/
    private void removeExchangeBetweenJoins(List<ExecNodeWrapper> orderedWrappers) {
        for (int i = 0; i < orderedWrappers.size(); i++) {
            ExecNodeWrapper wrapper = orderedWrappers.get(i);

            if (wrapper.execNode instanceof CommonExecExchange) {
                cutExchangeKeys(wrapper);
            }
            // match pattern: join-exchange-join,and remove the exchange
            if (wrapper.execNode instanceof StreamExecJoin) {
                cutJoinKeys(wrapper);
                // remove left exchange
                if (wrapper.inputs.get(0).execNode instanceof CommonExecExchange
                        && wrapper.inputs.get(0).inputs.get(0).execNode instanceof StreamExecJoin) {

                    ExecNodeWrapper targetJoinWrapper = wrapper;
                    ExecNodeWrapper sourceJoinWrapper = wrapper.inputs.get(0).inputs.get(0);
                    ExecNodeWrapper exchangeWrapper = wrapper.inputs.get(0);
                    ExecEdge oldInputEdge = wrapper.execNode.getInputEdges().get(0);
                    ExecEdge newInputEdge =
                            new ExecEdge(
                                    sourceJoinWrapper.execNode,
                                    targetJoinWrapper.execNode,
                                    oldInputEdge.getShuffle(),
                                    oldInputEdge.getExchangeMode());
                    targetJoinWrapper.execNode.replaceInputEdge(0, newInputEdge);
                    targetJoinWrapper.inputs.remove(0);
                    targetJoinWrapper.inputs.add(0, sourceJoinWrapper);
                    sourceJoinWrapper.outputs.remove(0);
                    sourceJoinWrapper.outputs.add(0, targetJoinWrapper);
                }
                // remove right exchange
                if (wrapper.inputs.get(1).execNode instanceof CommonExecExchange
                        && wrapper.inputs.get(1).inputs.get(0).execNode instanceof StreamExecJoin) {
                    ExecNodeWrapper targetJoinWrapper = wrapper;
                    ExecNodeWrapper sourceJoinWrapper = wrapper.inputs.get(1).inputs.get(0);
                    ExecNodeWrapper exchangeWrapper = wrapper.inputs.get(0);
                    ExecEdge oldInputEdge = wrapper.execNode.getInputEdges().get(1);
                    ExecEdge newInputEdge =
                            new ExecEdge(
                                    sourceJoinWrapper.execNode,
                                    targetJoinWrapper.execNode,
                                    oldInputEdge.getShuffle(),
                                    oldInputEdge.getExchangeMode());
                    wrapper.execNode.replaceInputEdge(1, newInputEdge);
                    targetJoinWrapper.inputs.remove(1);
                    targetJoinWrapper.inputs.add(1, sourceJoinWrapper);
                    sourceJoinWrapper.outputs.remove(0);
                    sourceJoinWrapper.outputs.add(0, targetJoinWrapper);
                }
            }
    ...
    private void cutJoinKeys(ExecNodeWrapper wrapper) {
        JoinSpec joinSpec = ((StreamExecJoin) wrapper.execNode).getJoinSpec();
        joinSpec.setLeftKeys(Arrays.copyOfRange(joinSpec.getLeftKeys(), 0, 1));
        joinSpec.setRightKeys(Arrays.copyOfRange(joinSpec.getRightKeys(), 0, 1));
        joinSpec.setFilterNulls(Arrays.copyOfRange(joinSpec.getFilterNulls(), 0, 1));
    }

    private void cutExchangeKeys(ExecNodeWrapper exchangeWrapper) {
        InputProperty.RequiredDistribution requiredDistribution =
                exchangeWrapper.execNode.getInputProperties().get(0).getRequiredDistribution();
        if (requiredDistribution instanceof InputProperty.HashDistribution) {
            int[] keys = ((InputProperty.HashDistribution) requiredDistribution).getKeys();
            if (keys.length > 1) {
                int[] newKey = Arrays.copyOfRange(keys, 0, 1);
                ((InputProperty.HashDistribution) requiredDistribution).setKeys(newKey);
            }
        }
    }
    ...
    private void optimizeMultipleInputJoinGroups(
    ...
    private List<ExecNodeWrapper> getInputWrappersInSameGroup(
            ExecNodeWrapper wrapper, MultipleInputJoinGroup group) {
    ...
    private StreamExecMultipleInputJoin createStreamMultipleInputJoinNode(
        ...
        StreamExecMultipleInputJoin MultipleInputJoin = null;
        try {
            MultipleInputJoin =
